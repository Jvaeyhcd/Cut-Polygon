...
//				}
			...
			if (!haveWriteDb) {
				...
				if (LevelActivity.lvManager.getStageByLevel(5).size() == 0){
					LevelActivity.lvManager.insertLevelInfo(5, removed, 1);
				}else if (LevelActivity.lvManager.getStageByLevel(5).size() == 1) {
					int oldScore = LevelActivity.lvManager.getStageByLevel(5).get(0).getScore();
					LevelActivity.lvManager.updateLevelInfo(5, oldScore > removed ? oldScore : removed, 1);
				} else {
					Log.i("Erro Message", "查出数据不是唯一的");
				}
				haveWriteDb = true;
