...
import com.hcd.jbox2d.game.activity.Stage3Activity;
...
public class Stage3View extends View {

	private Stage3View gameView;
	//过关百分比
	...
	public Stage3View(Context context, AttributeSet attrs) {
		super(context, attrs);
		paint = new Paint();
		...
		if (gameOver) {
			if (gameSuccess) {
				...
				if (!haveWriteDb) {
					if (LevelActivity.lvManager.getStageByLevel(4).size() == 0){
						LevelActivity.lvManager.insertLevelInfo(4, 0, 0);
					}else if (LevelActivity.lvManager.getStageByLevel(4).size() == 1) {
						//下一关有数据不用初始化
						//LevelActivity.lvManager.updateLevelInfo(4, 0, 0);
					} else {
			...
			if (!haveWriteDb) {
				...
				if (LevelActivity.lvManager.getStageByLevel(3).size() == 0){
					LevelActivity.lvManager.insertLevelInfo(3, removed, 1);
				}else if (LevelActivity.lvManager.getStageByLevel(3).size() == 1) {
					int oldScore = LevelActivity.lvManager.getStageByLevel(3).get(0).getScore();
					LevelActivity.lvManager.updateLevelInfo(3, oldScore > removed ? oldScore : removed, 1);
				} else {
					Log.i("Erro Message", "查出数据不是唯一的");
				}
				haveWriteDb = true;
	...
	private void initGame() {
		...
		lineNum = 3;
		cutArea = 0.0f;
		screenWidth = Stage3Activity.screenWidth;
		screenHeight = Stage3Activity.screenHeight;
	...
	private void createPlatform() {
		...
		platform = new Platform(world, (screenWidth / 2 + 2) / RATE, screenHeight * 1 / 16  / RATE, screenWidth * 11 / 20 / RATE, screenHeight * 7 / 16 / RATE);
		platforms.add(platform);
		platform = new Platform(world, screenWidth / 16 / RATE, screenHeight * 1 / 2 / RATE, screenWidth / 8 / RATE, screenHeight * 13 / 16 / RATE);
		platforms.add(platform);
	}
	...
	private void createPolygon() {
		...
		vecs[0] = new Vec2(screenWidth * 3 / 16 / RATE, (screenHeight) / 4 / RATE);
		vecs[1] = new Vec2(-screenWidth * 3 / 16 / RATE, (screenHeight) / 4 / RATE);
		vecs[2] = new Vec2(-screenWidth * 3 / 16 / RATE, -(screenHeight) / 4 / RATE);
		vecs[3] = new Vec2(screenWidth * 3 / 16 / RATE, -(screenHeight) / 4 / RATE);
